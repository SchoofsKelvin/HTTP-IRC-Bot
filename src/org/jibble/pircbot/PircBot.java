...
public abstract class PircBot implements ReplyConstants {
	...
	public final synchronized void connect(String hostname, int port, String password) throws IOException, IrcException, NickAlreadyInUseException {
		...
		if (isConnected()) { throw new IOException("The PircBot is already connected to an IRC server.  Disconnect first."); }
	...
	public final synchronized void sendRawLineViaQueue(String line) {
		if (line == null) { throw new NullPointerException("Cannot send null messages to server"); }
		if (isConnected()) {
			_outQueue.add(line);
		}
	}
	...
	public void changeNick(String newNick) {
		this.sendRawLineViaQueue("NICK " + newNick);
	}
	...
	protected void handleLine(String line) {
		...
		else if (command.equals("PRIVMSG") || command.equals("WHISPER")) {
	...
	public final void setMessageDelay(long delay) {
		if (delay < 0) { throw new IllegalArgumentException("Cannot have a negative time."); }
		_messageDelay = delay;
	}
	...
	public long ipToLong(byte[] address) {
		if (address.length != 4) { throw new IllegalArgumentException("byte array must be of length 4"); }
		long ipNum = 0;
		long multiplier = 1;
		for (int i = 3; i >= 0; i--) {
	...
	public int[] getDccPorts() {
		if (_dccPorts == null || _dccPorts.length == 0) { return null; }
	...
	private final User removeUser(String channel, String nick) {
		...
		synchronized (_channels) {
			Hashtable<?, ?> users = _channels.get(channel);
			if (users != null) { return (User) users.remove(user); }
		}
		return null;
	}
